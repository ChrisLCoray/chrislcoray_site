...
export default function Portfolio() {
    ...
    const portfolioData = [
        { name: 'ChrisLCoray.com', thumb_image: chrislcorayThumb, description: `It would make sense for a frontend developer to code their own website. This site was created using React, and in 2024 I've gone through and updated to React 18 with Bootstrap.`, code: '', link: '' },
    ...
    const portfolioRows = portfolioData.map(p =>
        <div className="row row-cols-5" key={`key-${p.name.replace(` `, '_').toLowerCase()}`}>
            ...
            <div className="col-sm-3 d-none d-md-block">{(p.link) ? <a href={p.link}>{p.link}</a> : "N/A"}</div>
            <div className="col-6 col-sm-3"><a href={p.code}>{p.code}</a></div>
